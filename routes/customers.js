...
const router= express.Router();
 ...
 router.get('/', async(req,res) =>{ 
      try{ const customer = await Post.find(); 
        res.json(customer);
  ...
  router.get('/:postid', async(req,res) =>{
      try{ const customer = await Post.findById(req.params.postid); 
        res.json(customer); 
    }catch(err){ res.json({message:err}); } })
 ...
 router.delete('/:postid', async(req,res) =>{ 
     try{ const removecustomer = await Post.deleteOne({_id: req.params.postid}); 
 ...
 router.patch('/:postid', async(req,res) =>{ 
    try{ const editcustomer = await Post.findOne({_id: req.params.postid}); 
    if(req.body.name){
